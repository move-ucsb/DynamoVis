...
public class InteractionsBuffer extends JPanel  {
	...
	/*public String[] getBufferDistances(){
		return distances;
	}//returns array of distance options in string form
	
	public String[] getBufferTimes(){
		return times;
	}//returns array of all time options in string form*/
	...
	public boolean withinDistanceBuffer(PointRecord p1, PointRecord p2, double buffer){	
		//pt_one.getDistance(pt_two) returns distance in kilometers between two points on earth
		double dist = gu.getDistance(p1.getLocation(),p2.getLocation());//returns distance in kilometers between two points on earth
		if(dist*1000<=buffer)
		{
			return true;//point is within buffer distance
		}
		else{return false;}//point too far	
	}//returns true if two passed in locations are within the buffer distance passed in, false otherwise
	
	public boolean withinTimeBuffer(PointRecord p1, PointRecord p2, double buffer){ 
		//the buffers in the animation do not disappear. This means that if an interaction occurs and one track keeps animating, but there isn't data for the other the
		//highlighted buffer remains. Buffers also may touch but unless the animals are with the others', interaction does not happen
	...
	}//returns true if times recorded in passed in PointRecord objects are within buffer(also passed in), false otherwise
	
	public ArrayList<PointRecord> calculateInteractingPoints(Track track1, Track track2, double bufferDist, double bufferTime){
		...
		for(int i = 0; i < p1.size(); i++){
			for(int j = 0; j<p2.size(); j++){
				if(withinDistanceBuffer(p1.get(i), p2.get(j),  bufferDist) && withinTimeBuffer(p1.get(i), p2.get(j), bufferTime))
		...
		return interacting;
	}//returns boolean array of all measured points, true if conditions met for interaction, false otherwise
	...
	public int getShortTrackLength(Track track1, Track track2){
		...
		else{ max = p1.size(); return p1.size();}	
