...
public class ActivitySpacePanel extends JPanel  {
	...
	public ActivitySpacePanel(DesktopPane father){
		...
		data = parent.data;
	   	...
	   	//at start of program assess if there are multiple animals or not ( give interacting options or not)
		...
		Draw.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (cb.isSelected()) {
					...
					data.boundary = false;	//if check box is not checked, set data.boundary to false (used in sketch.java)
				}
			}
		});	
		...
		Transparency_Slider.addChangeListener(new ChangeListener() {
	        public void stateChanged(ChangeEvent evt) {
	        	...
	        	data.boundaryColor = new Color(data.boundaryColor.getRed(),data.boundaryColor.getGreen(),data.boundaryColor.getBlue(),(int)Transparency_Slider.getValue());
	     		Transparency_TextField.setText(Integer.toString(data.boundaryColor.getAlpha()));
	        }
	    });
		...
		highlight_all.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (cb.isSelected()) {
					...
					data.highlight_all = true;//if check box is checked, set data.highlight_all to true (used in sketch.java)
					//data.highlight_fly = false;
				} else {
					//highlight_fly.setSelected(true);
					data.highlight_all = false;//if check box is not checked, set data.highlight_all to false (used in sketch.java)
				}
			}
		});	
		...
		highlight_fly.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (cb.isSelected()) {
					...
					//data.highlight_all = false;//if check box is checked, set data.highlight_fly to true (used in sketch.java)
				} else {
					//highlight_all.setSelected(true);
					data.highlight_fly = false;
					//data.highlight_all = true;//if check box is not checked, set data.highlight_fly to false (used in sketch.java)
				}
			}
		});
...
//					BufferChartPanel chart = new BufferChartPanel("Buffer Bar Chart", "Frequency of boundary visit", 
