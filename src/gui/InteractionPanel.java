...
public class InteractionPanel extends JPanel  {
	...
	{
		data.interactingPoints = ib.calculateInteractingPoints(data.track1, data.track2, data.bufferDist, data.bufferTime);//add bufferTime to enable
	...
	public InteractionPanel(DesktopPane father){
	    ...
	    //at start of program asks if there are multiple animals or not ( give interacting options or not)
	    if(parent.tagList.size()>1){data.multipleAnimals = true;}
	    else{data.multipleAnimals = false;}
	   	...
	   	if(data.multipleAnimals==true)
	   	{
		   	...
		   	data.selectedIDs = new String []{data.id1, data.id2};//create string of selected IDs (gotten from SketchData)
			...
			tagSelect1.addActionListener(new ActionListener () {
			    public void actionPerformed(ActionEvent e) {
	        		...
	        		data.selectedIDs[0] =(String)tagSelect1.getSelectedItem();//set 1st element in selected ID array to tag1 selection
	        		data.tagChange=true;
					data.interactingPoints = ib.calculateInteractingPoints(data.track1, data.track2, data.bufferDist, data.bufferTime);//add bufferTime to enable
					data.interactionsGenerated = true;
	        		data.upToDate = false;	
			...
			tagSelect2.addActionListener(new ActionListener () {
			    public void actionPerformed(ActionEvent e) {
	        		...
	        		data.selectedIDs[1] = (String)tagSelect2.getSelectedItem();//set second element in selected ID array to tag2 selection
					data.interactingPoints = ib.calculateInteractingPoints(data.track1, data.track2, data.bufferDist, data.bufferTime);//add bufferTime to enable
					data.interactionsGenerated = true;
					data.tagChange=true;
	        		data.upToDate = false;     
			...
			//checkbox for if highlights should be done on fly(makes buffer "ring" around lead point
			...
			//checkbox for highlightAll option(highlights all points that have been passed in timeline where interactions happened)
			...
			//checkbox for highlighting Boundary (shows the boundary colors the intersection and highlights it when both inside)
