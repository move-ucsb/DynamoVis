...
public class TimeBoxControlPanel extends JPanel{
	...
	//Sets up panel that user can label months, create static 3d visualization, and in future interaction
	...
	/*	
		...
		bdyInteraction.setForeground(Color.BLACK);		
		bdyInteraction.setFont(new Font("Arial", Font.PLAIN, 12));
		bdyInteraction.setMargin(new Insets(10, 5, 10, 10));
		this.add(bdyInteraction, "cell 0 3, gapleft 10");
		bdyInteraction.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent evt) {
				JCheckBox cb = (JCheckBox) evt.getSource();
				if (cb.isSelected()) {//sets boolean variable to true or false
					data.timeBoxBdyHighlight = true;
				} else {
					data.timeBoxBdyHighlight = false;
				}
			}
		});
