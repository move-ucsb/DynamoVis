...
//	public SpinnerModel edaySpinnerModel;
	...
	public BoundaryVisualizationPanel(DesktopPane father) {
		...
		//		setLayout(new MigLayout("insets 0", "[150px:150px:150px][50.00][150px:150px:150px,fill][][grow,fill][]", "[][][][grow,fill][][grow,fill][][][]"));
...
//		data.enable_check.setSelected(true);
		...
		Color_Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				final JColorChooser colorChooser = new JColorChooser(new Color(data.Bdy_Viz_Color.getRed(),data.Bdy_Viz_Color.getGreen(),data.Bdy_Viz_Color.getBlue(),data.Bdy_Viz_Color.getAlpha()));
				...
				{													
					...
					public void actionPerformed(
						...
						data.Bdy_Viz_Color = new Color(colorChooser.getColor().getRed(),colorChooser.getColor().getGreen(),colorChooser.getColor().getBlue(),data.Bdy_Viz_Color.getAlpha());
						Color_Button.setBackground(data.Bdy_Viz_Color);															
					}					
				}, new ActionListener()
				{
		...
		Transparency_Slider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent evt) {
				data.Bdy_Viz_Changes = true;
				data.Bdy_Viz_Color = new Color(data.Bdy_Viz_Color.getRed(),data.Bdy_Viz_Color.getGreen(),data.Bdy_Viz_Color.getBlue(),(int)Transparency_Slider.getValue());
				Transparency_TextField.setText(Integer.toString(data.Bdy_Viz_Color.getAlpha()));
			}
		});
		...
		Stroke_Color_Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				final JColorChooser Stroke_Color_Chooser = new JColorChooser(new Color(data.Bdy_Viz_Stroke_Color.getRed(),data.Bdy_Viz_Stroke_Color.getGreen(),data.Bdy_Viz_Stroke_Color.getBlue(),data.Bdy_Viz_Stroke_Color.getAlpha()));
				JDialog Stroke_maxDialog = JColorChooser.createDialog((Component) evt.getSource(),
						"Pick a Color",
						true,  //modal
						Stroke_Color_Chooser,
						new ActionListener()
				{													
					...
					public void actionPerformed(
							ActionEvent e) {
						data.Bdy_Viz_Changes = true;
						data.Bdy_Viz_Stroke_Color = new Color(Stroke_Color_Chooser.getColor().getRed(),Stroke_Color_Chooser.getColor().getGreen(),Stroke_Color_Chooser.getColor().getBlue(),data.Bdy_Viz_Stroke_Color.getAlpha());
						Stroke_Color_Button.setBackground(data.Bdy_Viz_Stroke_Color);															
					}					
				}, new ActionListener()
				{
		...
		Stroke_Transparency_Slider.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent evt) {
				data.Bdy_Viz_Changes = true;
				data.Bdy_Viz_Stroke_Color = new Color(data.Bdy_Viz_Stroke_Color.getRed(),data.Bdy_Viz_Stroke_Color.getGreen(),data.Bdy_Viz_Stroke_Color.getBlue(),(int)Stroke_Transparency_Slider.getValue());
				Stroke_Transparency_TextField.setText(Integer.toString(data.Bdy_Viz_Stroke_Color.getAlpha()));
			}
		});
		...
		Stroke_Weight_Spinner.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				data.Bdy_Viz_Changes = true;
				data.Bdy_Viz_Stroke_Weight= (int)Stroke_Weight_Spinner.getValue();
				//	            System.out.println("The selected StartYear in the TimeBar.java is:" + data.boundaryStartYear);
		...
		//	      
		//	      sdaySpinnerModel = new SpinnerNumberModel(data.startTime.getDayOfMonth(), //initial value
		//		         1, //min
		//		         31, //max
		//		         1);//step
		...
		//	      
		//	      edaySpinnerModel = new SpinnerNumberModel(data.startTime.getDayOfMonth(), //initial value
		//		         1, //min
		//		         31, //max
		//		         1);//step
		...
		//	      JButton generateBoundary = new JButton("Generate Boundary");//change color if displaying or not?
		...
		//				public void actionPerformed(ActionEvent evt) {
		//					//create array of booleans that tell true or false to if animals are interacting for entire length of time
