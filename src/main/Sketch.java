...
public class Sketch extends PApplet {
	...
	public void setup() {
		...
		//add raster
		//visImg = loadImage("./data/tiger7203_access_kde99ext_obsmexp2_threeClass1.png");
		//end raster

		//int containerWidth = (int) parent.sketchContainer.getContentPane().getSize().getWidth();
		//int containerHeight = (int) parent.sketchContainer.getContentPane().getSize().getHeight();
	...
	public void draw() {
			    ...
			    ArrayList<PointRecord> points = track.getPoints();		
			    	...
			    	for (int i = 0; i<points.size(); i++) { 
			    		...
			    		if (markerTime.isBefore(data.currentTime) || markerTime.isEqual(data.currentTime)) {			    			
			    			...
			    			if (data.ghost) ghost.vertex(pos.x,pos.y);
			    			...
			    			if (alpha !=0){
				    			...
				    			if (data.strokeColorToggle){
					    			...
					    			if (brushed) brush.vertex(pos.x,pos.y);					    			
				    			}
		...
		if (data.save){			
			String file = String.format("temp/" + parent.animationTitle + parent.exportCounter + "/temp%08d.jpeg",data.frameCounter);
			saveFrame(file);
			data.frameCounter++;
		}
