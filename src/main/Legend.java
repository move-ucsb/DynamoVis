...
public class Legend {
	...
	public void clicked(int mx, int my) {
		if (!data.legendLocked){			
		    ...
		    if (mx > x + 300 + lineColorX && mx < x + 300 + lineColorX + 290 && my > y-40 + lineColorY && my < y-20 + lineColorY ) {
		    	lineColorDrag = true;	      
		    	lineColorSetX = lineColorX-mx;
		    	lineColorSetY = lineColorY-my;
		    } else 
		    if (mx > x + 600 + lineWidthX && mx < x + 600 + lineWidthX + 290 && my > y-40 + lineWidthY && my < y-20 + lineWidthY ) {
		    	lineWidthDrag = true;	      
		    	lineWidthSetX = lineWidthX-mx;
		    	lineWidthSetY = lineWidthY-my;
		    } else
		    if (mx > x + 300 + pointColorX && mx < x + 300 + pointColorX + 290 && my > y-40 + pointColorY && my < y-20 + pointColorY ) {
		    	pointColorDrag = true;	      
		    	pointColorSetX = pointColorX-mx;
		    	pointColorSetY = pointColorY-my;
		    } else 
		    if (mx > x + 600 + pointSizeX && mx < x + 600 + pointSizeX + 290 && my > y-40 + pointSizeY && my < y-20 + pointSizeY ) {
		    	pointSizeDrag = true;	      
		    	pointSizeSetX = pointSizeX-mx;
		    	pointSizeSetY = pointSizeY-my;
		    } else
		    if (mx > x + 300 + vectorColorX && mx < x + 300 + vectorColorX + 290 && my > y-40 + vectorColorY && my < y-20 + vectorColorY ) {
		    	vectorColorDrag = true;	      
		    	vectorColorSetX = vectorColorX-mx;
		    	vectorColorSetY = vectorColorY-my;
		    } else 
		    if (mx > x + 600 + vectorLengthX && mx < x + 600 + vectorLengthX + 290 && my > y-40 + vectorLengthY && my < y-20 + vectorLengthY ) {
		    	vectorLengthDrag = true;	      
		    	vectorLengthSetX = vectorLengthX-mx;
		    	vectorLengthSetY = vectorLengthY-my;
		    } else
		    if (mx > x && mx < x + 890 && my > y-60 && my < y-40 ) {
		    	dragging = true;	      
		    	offsetX = x-mx;
		    	offsetY = y-my;
		    }
		...
		if (data.vectorColorToggle){
			if (data.vectorColorSelection.equals(parent.attributes.getIndex())){
				...
				for (int i = 0; i <= 200; i++) {
					...
					vectorColorLegend.stroke(parent.colors.coloursCont.get(data.selectedVectorSwatch).findColour((float) i/200));
	...
	private void drawPointColor() {
		...
		if (data.pointColorToggle){
			if (data.pointColorSelection.equals(parent.attributes.getIndex())){
				...
				for (int i = 0; i <= 200; i++) {
					//float percent = PApplet.norm(i, 0, 200);
					pointColorLegend.stroke(parent.colors.coloursCont.get(data.selectedPointSwatch).findColour((float) i/200));
					//legend.stroke(p.lerpColor(colorMin,colorMax, percent));
	...
	private void drawLineColor() {
		...
		if (data.strokeColorToggle){
			if (data.strokeColorSelection.equals(parent.attributes.getIndex())){
				...
				for (int i = 0; i <= 200; i++) {					
					lineColorLegend.stroke(parent.colors.coloursCont.get(data.selectedLineSwatch).findColour((float) i/200));					
